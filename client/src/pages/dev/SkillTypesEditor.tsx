...
import { uid } from "../../api";
import { Icons, SkillMark } from "../../components/icons";
...
export function SkillTypesEditor() {
  ...
  function add() {
    ...
    if (!l) return;
    update((d) => {
      d.skillTypes.push({ id: uid("st_"), label: l, icon, iconImage: null });
    });
    setLabel("");
    setIcon("star");
  }
    ...
    <div className="stack">
      <div className="ornate card">
        ...
        <p className="muted" style={{ marginTop: 0 }}>
          These power the proficiency grid (boons, banes, class weapons). Pick a built-in icon shape, or drop in your own
          icon image per type — an uploaded image overrides the built-in shape everywhere.
        </p>
        <div className="inline-add">
          ...
          <span className="prof-cell is-prof" style={{ width: 38, height: 38, flex: "none" }}><SkillMark type={{ icon }} /></span>
      ...
      <div className="ornate card">
        <div className="list-rows">
          {db.skillTypes.map((s) => (
            <div className="list-row ornate" key={s.id} style={{ alignItems: "flex-start" }}>
              <span className="prof-cell is-prof" style={{ width: 44, height: 44, flex: "none" }}><SkillMark type={s} size={24} /></span>
              <div className="grow">
                ...
                <div className="row" style={{ gap: 10, marginTop: 8, alignItems: "flex-start" }}>
                  <label className="field" style={{ margin: 0 }}>
                    <span>Built-in shape (fallback)</span>
                    <select value={s.icon} onChange={(e) => patch(s.id, { icon: e.target.value })} style={{ width: 150 }}>
                      {ICON_KEYS.map((k) => <option key={k} value={k}>{k}</option>)}
                    </select>
                  </label>
                  <div style={{ width: 130 }}>
                    <label className="field" style={{ margin: 0 }}><span>Icon image</span></label>
                    <ImageDrop value={s.iconImage ?? null} onChange={(url) => patch(s.id, { iconImage: url })} height={72} label="Drop icon" />
                  </div>
                </div>
              </div>
